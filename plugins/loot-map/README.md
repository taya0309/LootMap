...
| **「珠宝匠/工匠」蓝星不亮或不准确** | 前缀编号（1209/1210）按实测环境校准；换模组后编号可能变化，需重新校准 |
...
- rule0 蓝装前缀判据（前缀 ID 1209/1210）按实测环境校准；原版或其他模组可能不同
- 游戏大版本更新可能改变内存偏移，需要重新校准（校准/校验脚本为开发期内部工具，未随仓库公开）
